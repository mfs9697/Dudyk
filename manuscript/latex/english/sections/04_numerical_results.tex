...
Cases with $E_1/E_2>1$ are not independent. They are recovered by
interchanging the materials together with
\begin{equation*}
  ...
  1\leftrightarrow2.
\end{equation*}
For $\nu_1=\nu_2$, this includes the replacement
...
the load simultaneously increases the local deformation and energy
parameters and moves the model closer to the limit of its asymptotic
applicability.
...
length $d_i$. Consequently, the deformation criterion
$\delta_i=\delta_{i\mathrm{c}}$ and the energy criterion
$J_i=J_{i\mathrm{c}}$ provide two representations of the same
one-parameter family of local equilibrium states. They are equivalent
only when the independently specified critical material parameters
$\delta_{i\mathrm{c}}$ and $J_{i\mathrm{c}}$ are chosen consistently;
the present curves are not two independent evolution laws.
