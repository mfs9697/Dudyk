...
one-parameter family of local equilibrium states. They are equivalent only when the independently specified critical material
parameters $\delta_{i\mathrm{c}}$ and $J_{i\mathrm{c}}$ are chosen
consistently. Thus, the two sets of curves are alternative measures of the
same equilibrium family rather than independent evolution laws.
...
$d_i/l\ll1$. The full curve is therefore useful for examining the structure of the
solution, but quantitative interpretation near the largest maximum should
account for this limitation of the small-scale approximation.
...
For comparison, consider three local configurations: the intact bimaterial
corner, the corner containing the prescribed interfacial shear crack, and
the configuration containing both the crack and the process zone. For the
...
materials, but the existence of a process zone is not governed by the
singularity exponent alone. It also depends on the conditions $Cg_2<0$ and $CQ_i>0$, the
