...
order $r^{\lambda_i}$, so $\Phi_i^-$ is analytic for
...
\begin{equation*}
  ...
  <\operatorname{Re}p<0.
\end{equation*}
Here, $\operatorname{Re}p$ denotes the real part of $p$. The subsequent
Wiener--Hopf separation uses analytic continuation to the boundary
$\operatorname{Re}p=0$: $\Phi_i^+$ is continued from the left and
$\Phi_i^-$ from the right. The kernel $G_i(p)$ is
\begin{equation*}
