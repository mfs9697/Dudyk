...
Consider a piecewise homogeneous body under plane strain, consisting of
two dissimilar isotropic elastic materials joined along a V-shaped interface.
...
The corner of the cracked interface retains a power-law stress
singularity~\cite{NazarenkoKipnis2022Stress}. This field can support a
process zone in one of the materials, with subsequent formation of an
opening-mode (Mode~I) crack. Under the assumed symmetry of the geometry
and loading, the bisector of the sector occupied by that material is a
natural direction for the zone. Along this direction, the local in-plane
shear-mode (Mode~II) component at the zone tip vanishes by symmetry,
...
The process zone in material~$i$ is modeled as a line of normal displacement
discontinuity of length $d_i$, subject to a constant normal cohesive
traction $\sigma_i>0$, representing the material's resistance to
...
asymptotic field of the interfacial crack without a process
...
known no-zone field and introduce the correction normal stress
\begin{equation*}
  \widehat{\sigma}_i(r)
...
\end{equation*}
The no-zone field satisfies the bisector symmetry condition $u_\theta=0$,
so subtracting it does not alter the displacement-gradient transform used
below. Thus, the functional problem is formulated for the correction field,
with the known power-law term entering its prescribed part.
...
Near the outer tip of the displacement discontinuity, the stress and
normal displacement gradient have the standard inverse-square-root
asymptotic form for a homogeneous material:
\begin{equation}
...
zone and from within it, respectively. The boundary-value problem is
closed by imposing the stress-regularity condition standard for this
