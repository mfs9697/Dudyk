...
For the intact interface, the singular part of the interfacial stress
field is~\cite{NazarenkoKipnis2022Stress}
\begin{equation}
...
\end{aligned}
\end{equation}
where $C$ is the generalized stress intensity factor, $\lambda_0$ is
...
\end{aligned}
\end{equation*}
\subsection{Corner with an interfacial shear crack}

For the prescribed symmetric interfacial shear crack, each branch has
length $l$. At distances $r\ll l$, the normal stress along the bisector
...
$G^+$ belongs to the base kernel $G$, distinct from the zone kernel
$G_i$ in Section~\ref{sec:derivation}. Explicitly,
\begin{equation*}
\begin{aligned}
...
containing the interfacial crack satisfies
\begin{equation}
\label{eq:app-D-root}
  D(-1-\lambda)=0,
  \qquad -1<\lambda<0,
...
\end{equation*}
Here, $z$ is the complex integration variable, $\ii^2=-1$, and the
contour runs upward along the imaginary axis. The gamma-function factor is
\begin{equation*}
  K^+(p)=\frac{\Gamma(1-p)}{\Gamma(\tfrac12-p)}.
\end{equation*}
where $\Gamma$ is the Euler gamma function. For real $p<0$, the
verified numerical algorithm evaluates $G^+(p)$ equivalently using
the real integral
\begin{equation*}
  \log G^+(p)
...
\end{equation*}
where $t$ is a real integration variable.
...
\label{eq:app-post-zone-root}
  ...
  \qquad i=1,2.
\end{equation}
These two roots are used for the $\lambda_1$ and $\lambda_2$ curves in
\SingularityExponentFigure. They characterize the corner singularity
in the presence of a process zone and must not replace $\lambda$ in
the formulas for $d_i$, $\delta_i$, and $J_i$, where $\lambda$ is
defined by Eq.~\eqref{eq:app-D-root}.
