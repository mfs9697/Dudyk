\section{Визначення параметрів рівноважної зони передруйнування}
...
\end{aligned}
\end{equation*}
Зокрема, показник сингулярності у конфігурації із зоною
передруйнування в матеріалі $i$ визначається рівнянням
\begin{equation*}
  D_i(-1-\lambda_i)=0,
\end{equation*}
...
Для ядер, що використовуються далі, чисельно перевірено, що
$G_i(\ii t)$ є дійсним, додатним і парним за $t$ та прямує до одиниці
за $|t|\to\infty$. Тому для дійсного $p<0$ числове обчислення
...
підрозділі~\ref{subsec:boundary-conditions}. За незмінної геометрії та
фіксованої довжини гілки V-подібної міжфазної тріщини $l$
формула~\eqref{eq:process-zone-length} дає степеневий закон
$d_i\propto |C|^{-1/\lambda}$. Отже, зі зміною модуля зовнішнього
навантаження $|C|$ розглядається квазістатична сім'я рівноважних станів
зони $d_i(C)$, тоді як довжина кожної гілки V-подібної міжфазної
тріщини залишається незмінною. В ідеалізованій сингулярній постановці
формула не задає скінченного порогу появи самої зони: для будь-якого
ненульового фізично допустимого $C$ маємо $d_i>0$, причому $d_i\to0$
за $|C|\to0$. Критичний матеріальний параметр входить на наступному
етапі --- під час визначення навантаження ініціювання відгалуженої
тріщини за характеристиками рівноважної зони.

Якщо, навпаки, фіксувати геометрію, довжину гілки $l$ V-подібної
міжфазної тріщини та зовнішнє навантаження, то з тієї самої формули випливає залежність від
...
$\delta_i=\jump{u_\theta(0,\beta_i)}$. Зірочкою нижче позначаємо
перетворення Мелліна відповідної похідної. Із означення $\Phi_i^-(p)$
випливає
\begin{equation*}
  \delta_i
  =-2
...
\begin{equation*}
  ...
  {4(1-\nu_i^2)\sigma_i^2l}.
\end{equation*}
Штрих у $\sigmap$, $\delta_i'$ і $J_i'$ позначає нормування, а не
диференціювання. Тоді формули~\eqref{eq:normalized-process-zone-length},
...
$D(-1-\lambda)=0$. Показники $\lambda_1$ і $\lambda_2$, що описують сингулярність у
конфігураціях із зоною передруйнування, не підставляються замість
$\lambda$ у формули її параметрів.

За фіксованих геометрії та властивостей матеріалу як $\delta_i$, так і
$J_i$ лінійно залежать від тієї самої рівноважної довжини $d_i$.
Виключаючи $d_i$ з
\eqref{eq:process-zone-opening} і
\eqref{eq:process-zone-energy-parameter}, вводимо безрозмірний
коефіцієнт пропорційності $\chi_i$:
\begin{equation}
...
міжфазних тріщин~\cite{KaminskyEtAl2023}, ініціювання відгалуженої
тріщини можна описувати енергетичною умовою
\begin{equation*}
  J_i=J_{i\mathrm{c}},
\end{equation*}
...
За фіксованих геометрії та властивостей матеріалів
співвідношення~\eqref{eq:energy-opening-relation} показує, що ці два
критерії описують ту саму однопараметричну сім'ю станів і вибирають той
самий критичний стан лише за узгодженого калібрування незалежно заданих
