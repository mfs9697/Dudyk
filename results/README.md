...
`figure2_recalculated.csv` is generated by `generate_figure2` after the same
test suite passes. It uses long format with `case_id`, `alpha_deg`,
`material_index`, `sigma_prime`, `d_over_l`, `delta_prime`, and `J_prime`.
There are 201 load rows for each of the four original Figure-2 cases. The
stored values are not clipped to the manuscript's plotting window.
