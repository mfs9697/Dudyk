...
% Springer Nature sn-jnl, iicol, Math/Physical Sciences author-year style.
...
\end{aligned}
\end{equation}
The condition $\jump{u_\theta}=0$\end{strip}
 imposes normal contact between the crack
...
\end{aligned}
\end{equation}
Here, $k_i$ is the local Mode~I\end{strip}
 stress intensity factor at the process-zone
...
\end{equation}
which is initially valid in the common convergence strip\end{strip}
...
\end{aligned}
\end{equation*}
In particular, the corner singularity exponent\end{strip}
 $\lambda_i$ in the
...
\begin{equation*}
  ...
  \end{cases}
\end{equation*}
where $z$ is the complex integration variable\end{strip}
 and $\ii^2=-1$.
