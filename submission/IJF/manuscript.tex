...
\begin{aligned}
  &\theta=\beta_i,\quad r\to d_i+0:\\[-2pt]
  &\qquad \sigma_\theta(r,\beta_i)
    \sim \frac{k_i}{\sqrt{2\pi(r-d_i)}},\\[3pt]
  &\theta=\beta_i,\quad r\to d_i-0:\\[-2pt]
  &\qquad \frac{\partial u_\theta}{\partial r}(r,\beta_i)
    \sim -\frac{2(1-\nu_i^2)}{E_i}
...
\begin{aligned}
  ...
  &\times\{\sin^2[p(\pi-\alpha)]-p^2\sin^2\alpha\}\\
  &+(1+\kappa_1)
    [\sin(2p\alpha)+p\sin(2\alpha)]\\
  &\times[\sin(2p(\pi-\alpha))-p\sin(2\alpha)],\\[2pt]
  D_2(p)={}&2(1+\kappa_1)
    [\cos(2p(\pi-\alpha))-\cos(2\alpha)]\\
  &\times[\sin^2(p\alpha)-p^2\sin^2\alpha]\\
  &+e(1+\kappa_2)
    [\sin(2p(\pi-\alpha))-p\sin(2\alpha)]\\
  &\times[\sin(2p\alpha)+p\sin(2\alpha)].
\end{aligned}
\end{equation*}
...
\Bigg]
\Bigg\},\\[-1pt]
&\hspace{13mm}\operatorname{Re}p<0,\\[4pt]
\Phi_i^-(p)={}&K^-(p)G_i^-(p)
\Bigg[
\frac{\sigma_iK^+(-1)}
{(p+1)G_i^+(-1)}\\
...
{(\lambda+1)G_i^+(-\lambda-1)}
\Bigg],\\[-1pt]
&\hspace{13mm}\operatorname{Re}p>0.
\end{aligned}
\end{equation}
}
...
\begin{aligned}
  ...
  &\times\bigl[(\lambda_0+1)\sin2\alpha
...
\begin{aligned}
  b_0(p)={}&
  [\sin(2p\alpha)+p\sin(2\alpha)]\\
  &\times[\kappa_1\sin\{2p(\pi-\alpha)\}+p\sin(2\alpha)],\\[2pt]
  b_1(p)={}&(1+\kappa_1)(1+\kappa_2)\sin^2(p\pi)\\
  &-[\sin(2p\alpha)+p\sin(2\alpha)]\\
  &\quad\times[\kappa_1\sin\{2p(\pi-\alpha)\}+p\sin(2\alpha)]\\
  &-[\sin\{2p(\pi-\alpha)\}-p\sin(2\alpha)]\\
  &\quad\times[\kappa_2\sin(2p\alpha)-p\sin(2\alpha)],\\[2pt]
  b_2(p)={}&
  [\sin\{2p(\pi-\alpha)\}-p\sin(2\alpha)]\\
  &\times[\kappa_2\sin(2p\alpha)-p\sin(2\alpha)].
\end{aligned}
\end{equation*}
}
...
\begin{equation*}
\begin{aligned}
  S_1(p)={}&2e(1+\kappa_2)s_{11}(p)
  \sin[(p+1)\alpha]\\
  &+s_{12}(p)(1+\kappa_1)
  \sin\{p(\pi-\alpha)-\alpha\}\\
  &+2(1-e)s_{12}(p)s_{13}(p),\\[2pt]
  S_2(p)={}&e(1+\kappa_2)s_{21}(p)
  \sin[(p+1)\alpha]\\
  &+2(1-e)s_{21}(p)s_{22}(p)\\
  &-2(1+\kappa_1)
  \sin\{p(\pi-\alpha)-\alpha\}\,s_{23}(p).
\end{aligned}
\end{equation*}
}
...
\begin{aligned}
  ...
  &\times[\sin(2p\alpha)+p\sin2\alpha],\\
  a_1(p)={}&(1+\kappa_2)
  [\cos(2p\alpha)-\cos2\alpha]\\
  &\times[\sin\{2p(\pi-\alpha)\}-p\sin2\alpha].
\end{aligned}
\end{equation*}
}
...
\begin{equation*}
  G(p)=
  \frac{(e+\kappa_1)(1+e\kappa_2)}
  {e+\kappa_1+1+e\kappa_2}
  \frac{\sin(\pi p)D(p)}
  {\cos(\pi p)D_0(p)}.
\end{equation*}
and its plus factor
...
\begin{aligned}
  ...
  \right\},\\[-1pt]
  &\hspace{17mm}\operatorname{Re}p<0.
\end{aligned}
\end{equation*}
}
...
\begin{aligned}
  ...
  &\times[\sin\{2p(\pi-\alpha)\}-p\sin2\alpha],\\[3pt]
  D_2(p)={}&2(1+\kappa_1)
  [\cos\{2p(\pi-\alpha)\}-\cos2\alpha]\\
  &\times[\sin^2(p\alpha)-p^2\sin^2\alpha]\\
  &+e(1+\kappa_2)
