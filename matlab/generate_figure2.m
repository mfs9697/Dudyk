...
% The complete view prevents the 45-degree and 10-degree corrected
% opening/energy curves from being hidden by the manuscript crop.
...
fprintf(['Author-validation note: the author confirmed the material-index ', ...
    'correction in August and the opening/energy formula corrections on ', ...
    '25 September 2026. The August Mathcad opening and energy values are ', ...
    'historical; current publication values are those written by this run. ', ...
    'See docs/AUTHOR_CONFIRMED_FORMULA_CORRECTIONS_2026-09-25.md.\n']);
end

function write_figure(caseData, cases, yMaximum, figuresDir, stem)
% Compact 3:1 canvas: at manuscript text width this keeps the three-panel
% figure substantially shorter while retaining readable vector lettering.
...
legendLabels = figure2_legend_labels(cases);
...
for panel = 1:3
    ...
    for k = 1:numel(cases)
        handle = plot(ax, caseData{k}.sigma_prime, ...
            ...
            'Color', cases(k).color, 'LineWidth', 1.8);
        if panel == 1
...
xlabel(ax, '$\sigma^{\prime}$', 'Interpreter', 'latex', ...
    'FontSize', 11.5);
ylabel(ax, yLabelText, 'Interpreter', 'latex', ...
    'FontSize', 11.5);
end

function labels = figure2_legend_labels(cases)
...
for k = 1:numel(cases)
    ...
    labels{k} = sprintf( ...
        '$\\alpha=%g^{\\circ},\\;\\sigma^{\\prime}%s0,\\;i=%d$', ...
        cases(k).alpha_deg, signText, cases(k).material_index);
end
end
